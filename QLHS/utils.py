...
def get_mark_subject_of_student(student_id, subject_id, id_hocKy):
    return Hoc.query.filter(Hoc.student_id == student_id, Hoc.subject_id == subject_id, Hoc.id_hocKy == id_hocKy).first()
...
def diem_trung_binh_lop(class_id):
    ...
    for st in list_student:
        dict.append({
            ...
            'ca_nam': "{:.1f}".format((diem_TB_HK(student_id=st.id, id_hocKy=1) + diem_TB_HK(student_id=st.id, id_hocKy=2)*2)/3),
            'xep_loai': xap_loai(float("{:.1f}".format((diem_TB_HK(student_id=st.id, id_hocKy=1) + diem_TB_HK(student_id=st.id, id_hocKy=2)*2)/3)))
        })
        i = i + 1
    return dict
